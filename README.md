...
The exchange-rate data allows sales originally recorded in different currencies to be standardized to EUR for financial analysis.
...
Provides an executive summary of Nova Retail performance, including:

Total Revenue
Total Profit
Total Orders
Average Order Value
Distance to Monthly Target
Revenue Trend & Moving Average
Revenue by Product Category
Category-to-Product drill-down
Interactive filters

The trend visual supports both daily and weekly analysis through a time-granularity parameter and includes a dynamic moving average controlled by the selected number of days.

Analysis

Provides time-intelligence and target analysis, including:
...
The dashboard allows users to select a reporting month and interact with product category, region, time granularity and moving average parameters.

Because the available sales data only covers 2023, a Previous YTD comparison cannot be calculated from the available dataset.

Target vs Actual Analysis

Actual revenue is represented by the Total Revenue measure from FCT_SALES.

Target revenue is represented by the Total Target measure from FCT_TARGETS.

The dashboard compares actual revenue against target revenue by month and provides a visual indicator showing the distance from the monthly target.

Regional target analysis is available through FCT_TARGETS.

Actual sales by region cannot be calculated because the sales source does not contain regional information.
...
Therefore, category-based target comparisons are not directly linked to the sales category filter in the current model.

Time Intelligence

The dashboard implements month selection and time-intelligence calculations for MTD and YTD analysis.

The dataset contains sales data for 2023 only. As a result, Previous YTD cannot be calculated because there is no 2022 sales data available.
...
The project combines Dataiku preparation, dbt transformations and automated dbt tests to ensure that the analytical model is consistent and reliable.
...
Target grain validation
FX grain validation
Positive target amounts
EUR conversion consistency

All implemented dbt tests pass successfully.
